...
contract('Exchange', ([deployer, feeAccount, user1, user2]) => {
  ...
  describe('deployment', () => {
    ...
    it('tracks admin Account', async() => {
      const result = await exchange.admin();
  ...
  describe('depositing Ether', () => {
    ...
    it('emits a Deposit event', () => {
      ...
      event.amount.toString().should.equal(amount.toString(), 'amount ether in correct')
      event.balance.toString().should.equal(amount.toString(), 'balance correct')
    })
  })
  ...
  describe('depositing tokens', () => {
    ...
    describe('failure', () => {
      it('rejects Ether deposits', () => {
        exchange.depositToken(ETHER_ADDRESS, tokenFormat(10), { from: user1 }).should.be.rejectedWith(EVM_REVERT)
      })

      it('fails when no tokens are approved', () => {
        // Tokens need approve before depositing
        exchange.depositToken(token.address, tokenFormat(10), { from: user1 }).should.be.rejectedWith(EVM_REVERT)
      })
    })
  })
  ...
  describe('withdrawing tokens', () => {
    ...
    describe('failure', () => {
      it('rejects Ether withdraws', () => {
        exchange.withdrawToken(ETHER_ADDRESS, tokenFormat(10), { from: user1 }).should.be.rejectedWith(EVM_REVERT)
      })

      it('fails for insufficient balances', () => {
        // Attempt to withdraw tokens without depositing any first
        exchange.withdrawToken(token.address, tokenFormat(10), { from: user1 }).should.be.rejectedWith(EVM_REVERT)
      })
    })
  })
  ...
  describe('making orders', () => {
    ...
    beforeEach(async () => {
      result = await exchange.makeOrder(token.address, tokenFormat(1), ETHER_ADDRESS, etherFormat(1), { from: user1 })
    })
    ...
    it('tracks the newly created order', async () => {
      ...
      order.amountGet.toString().should.equal(tokenFormat(1).toString(), 'amountGet is correct')
      order.tokenGive.should.equal(ETHER_ADDRESS, 'tokenGive is correct')
      order.amountGive.toString().should.equal(etherFormat(1).toString(), 'amountGive is correct')
    ...
    describe('filling orders', () => {
      ...
      describe('success', () => {
        ...
        it('executes the trade & may charges fees', async () => {
          ...
          balance.toString().should.equal(tokenFormat(1).toString(), 'user1 received tokens')
          ...
          if(isLucky) { 
            ...
            const feeAccount = await exchange.feeAccount()
            balance = await exchange.balanceOf(token.address, feeAccount)
            balance.toString().should.equal(tokenFormat(0.01).toString(), 'feeAccount received fee')
          }          
  ...
  describe('circuit breaker ..emergency', () => {
    ...
    describe('success', async () => {
        ...
        it('it rejects new orders', async () => {
            exchange.makeOrder(token.address, tokenFormat(1), ETHER_ADDRESS, etherFormat(1), { from: user1 }).should.be.rejectedWith(EVM_REVERT)
        })
        ...
        it('it denies ETH deposits', async () => {
            exchange.depositEther({ from: user1, value: etherFormat(1) }).should.be.rejectedWith(EVM_REVERT)
        })

        it('it denies Token deposits', async () => {
            exchange.depositToken(token.address, tokenFormat(2), { from: user2 }).should.be.rejectedWith(EVM_REVERT)
        })
        ...
        describe('success admin can restart all exchange functionality', async () => {
            ...
            describe('resumes all exchange functionality', () => {
                ...
                it('allows ETH deposit', () => {
                    exchange.depositEther({ from: user1, value: etherFormat(1) }).should.be.fulfilled
                })

                it('allows Token deposits', async () => {
                  exchange.depositToken(token.address, tokenFormat(1), { from: user2 }).should.be.fulfilled
                })

                it('allows Making orders', () => {
                    exchange.makeOrder(token.address, tokenFormat(1), ETHER_ADDRESS, etherFormat(1), { from: user1 }).should.be.fulfilled
                })
